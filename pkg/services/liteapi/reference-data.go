...
// func (r *referencedatasvcs) GetHotelFacilities(ctx context.Context) (*models.FacilityList, error) {
// 	pipeline := []bson.M{
// 		{"$match": bson.M{"expiresAt": bson.M{"$gt": time.Now().UTC()}}},
// 	}

// 	var result []models.Facility
// 	err := r.facilityrepo.Aggregate(ctx, pipeline, func(cursor *mongo.Cursor) error {
// 		return cursor.All(ctx, &result)
// 	})
// 	if err != nil || len(result) == 0 {
// 		liteApiSdk, err := r.liteApiInitFunc(ctx)
