...
func (h *FaveHandler) GetAllByType(w http.ResponseWriter, r *http.Request) error {
	...
	switch typeStr {
	case string(models.FaveTypeProgram), string(models.FaveTypeHotel), string(models.FaveTypeDiary), string(models.FaveTypeExhibition), string(models.FaveTypeAgent), string(models.FaveTypeDestination):
		// valid
	default:
...
func (h *FaveHandler) Add(w http.ResponseWriter, r *http.Request) error {
	...
	objId, err := primitive.ObjectIDFromHex(rawData.RefId)
	if err != nil {
		log.Println("Error converting refId to ObjectID:", err, rawData.RefId)
		return helpers.BadRequest("Invalid refId format")
	}
