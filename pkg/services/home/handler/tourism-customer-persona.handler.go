...
func (h *CustomerPersonaHandler) GetV2(w http.ResponseWriter, r *http.Request) error {
	ctx, _ := util.AddCtxAppCfg(r)
	skip, limit, err := util.Paginate(r)
	if err != nil {
		return err
	...
	if err := json.NewDecoder(r.Body).DecodeContext(ctx, &query); err != nil {
		return helpers.InvalidJSON()
	}

	result, err := h.personaSvcs.GetV2(ctx, skip, limit, &query)
	if err != nil {
		return err
	}
	return helpers.WriteJsonCtx(ctx, w, http.StatusOK, result)
...
func (h *CustomerPersonaHandler) GetAllV2(w http.ResponseWriter, r *http.Request) error {
	ctx, _ := util.AddCtxAppCfg(r)
	var query query.Conditions
	if err := json.NewDecoder(r.Body).DecodeContext(ctx, &query); err != nil {
		return helpers.InvalidJSON()
	}
	result, err := h.personaSvcs.GetAllV2(ctx, &query)
	if err != nil {
		return err
	}
	return helpers.WriteJsonCtx(ctx, w, http.StatusOK, result)
